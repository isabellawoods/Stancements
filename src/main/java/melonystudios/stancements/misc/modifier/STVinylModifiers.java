        ...
        context.register(MUSIC_DISC_11, VinylModifier.modifier(HolderSet.direct(jukeboxSongs.getOrThrow(JukeboxSongs.ELEVEN)))
                ...
                .withSpecialModifier(STModifierComponents.REPLACE_RECORDED_DISC.get(), List.of(
                        new ConditionalEffect<>(
                                ...
                                Optional.of(new ItemRecorderStateCondition(HolderSet.direct(STItems.VINYL_DISC.getDelegate())))
                        ),
                        new ConditionalEffect<>(
                                new ReplaceRecordableDiscModifier(STItems.SCULK_INFESTED_SHATTERED_DISC.toStack()),
                                Optional.of(new ItemRecorderStateCondition(HolderSet.direct(STItems.SCULK_INFESTED_VINYL_DISC.getDelegate())))
                        )
                ))
