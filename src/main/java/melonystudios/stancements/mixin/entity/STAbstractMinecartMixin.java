...
public abstract class STAbstractMinecartMixin extends VehicleEntity {
    ...
    public float maintainSpeedAfterLeavingGilded(float railMaxSpeed, @Local BlockState state, @Local BlockPos pos) {
        ...
        return this.gildedSpeedBuildup > 0 ? rail.getRailMaxSpeed(state, this.level(), pos, this.minecart()) * (rail.getRailDirection(state, this.level(), pos, this.minecart()).isSlope() ? 1 : STOptions.GILDED_RAIL_SPEED_MULTIPLIER.get().floatValue()) : railMaxSpeed;
    }
